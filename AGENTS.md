...
根目录当前包含论文资料、论文数据和本说明文件。除 `AGENTS.md` 外，现有资料均应视为只读输入。

| 路径 | 类型 | 大小 | 已知信息 | 使用注意 |
| --- | --- | ---: | --- | --- |
| `AGENTS.md` | Markdown 文档 | 8,677 bytes | 项目协作与复现说明 | 可按项目进展持续补充；文件大小会随内容更新变化 |
...
| `论文数据/Figure5.xlsx` | Excel 2007+ | 11,720 bytes | 1 个工作表：`Sheet1`；sheet XML 中有 50 个 row 标签 | 用于 Figure 5 数据复核和图表复现 |
| `论文数据/Figure6.xlsx` | Excel 2007+ | 13,019 bytes | 1 个工作表：`Sheet1`；sheet XML 中有 63 个 row 标签 | 用于 Figure 6 数据复核和图表复现 |
| `论文数据/Figure7.xlsx` | Excel 2007+ | 13,247 bytes | 1 个工作表：`Sheet1`；sheet XML 中有 87 个 row 标签 | 用于 Figure 7 数据复核和图表复现 |
| `论文数据/Figure8.xlsx` | Excel 2007+ | 18,267 bytes | 1 个工作表：`Sheet1`；sheet XML 中有 87 个 row 标签 | 用于 Figure 8 数据复核和图表复现 |
...
3. 对 `Figure5.xlsx` 至 `Figure8.xlsx` 做数据审计，记录每个工作表、列名、单位和对应论文图号。
4. 对 `microCT_Berea.raw` 做只读检查，确认形状、数据类型、取值范围和相标签含义；若论文未明确说明，不要擅自假定，需在笔记中标为待确认。
5. 先实现最小可验证模块，例如数据读取、相标签统计、简单体素可视化、边界条件构造，再逐步实现完整模拟。
6. 每完成一个复现步骤，应生成与论文结果的对比记录，包括图表、误差指标或解释性说明。
...
- 如需新增依赖，优先安装到 `sip-simpeg` 环境中，并记录依赖名称、用途和安装方式。
